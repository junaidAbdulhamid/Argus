...
from app.models.entities import Project, Task, TaskStatus, Role, User
from app.models.quality import AnnotationSchema, Review, Escalation, GoldReference
...
def review_queue(project_id: str | None = None, status: str | None = None, user=Depends(reviewer), db=Depends(get_db)):
    q = (
        ...
        .where(
            ...
            Task.status.in_([TaskStatus.PENDING_REVIEW, TaskStatus.ESCALATED, TaskStatus.CHANGES_REQUESTED]),
        )
    )
    if project_id:
    ...
    return [
        {**serialize(t), "consensus": calculate(db, t, store=False), "project_name": db.get(Project, t.project_id).name}
        for t in db.scalars(q.order_by(Task.updated_at).limit(200))
    ]
...
def task_quality(task_id: str, user=Depends(reviewer), db=Depends(get_db)):
    ...
    return {
        "consensus": calculate(db, t),
        "policy": policy(db.get(Project, t.project_id)),
        "schema": schema_detail(db, db.get(AnnotationSchema, t.schema_id)) if t.schema_id else None,
        "reviews": [
            {**serialize(r), "reviewer": db.get(User, r.reviewer_id).full_name}
            for r in db.scalars(select(Review).where(Review.task_id == t.id).order_by(Review.created_at.desc()))
        ],
        "escalations": [
            serialize(e)
            for e in db.scalars(
                select(Escalation).where(Escalation.task_id == t.id).order_by(Escalation.created_at.desc())
            )
        ],
    }
...
def review(task_id: str, data: ReviewCreate, user=Depends(reviewer), db=Depends(get_db)):
    return serialize(
        submit_review(db, user, task(db, user, task_id, lock=True), data.decision, data.comments, data.metadata)
    )
...
def escalation(task_id: str, data: EscalationInput, user=Depends(current_user), db=Depends(get_db)):
    return serialize(
        escalate(
            db,
            user,
            task(db, user, task_id, lock=True),
            data.reason,
            "ANNOTATOR" if user.role == Role.ANNOTATOR else "REVIEWER",
        )
    )
...
def inbox(
    ...
    q = (
        ...
        .join(Task)
        .join(Project)
        .where(Project.organization_id == user.organization_id, Escalation.status == status)
    )
    if project_id:
        q = q.where(Task.project_id == project_id)
...
def resolution(escalation_id: str, data: EscalationResolution, user=Depends(reviewer), db=Depends(get_db)):
    e = db.scalar(
        ...
        .join(Task)
        .join(Project)
        .where(Escalation.id == escalation_id, Project.organization_id == user.organization_id)
    )
    if not e:
...
def gold(task_id: str, data: GoldInput, user=Depends(admin), db=Depends(get_db)):
    ...
    if "score" in data.expected and (
        type(data.expected["score"]) not in (int, float) or not 1 <= data.expected["score"] <= 5
    ):
...
def gold_list(user=Depends(admin), db=Depends(get_db)):
    return [
        {**serialize(g), "external_id": t.external_id, "project_id": t.project_id}
        for g, t in db.execute(
            select(GoldReference, Task).join(Task).join(Project).where(Project.organization_id == user.organization_id)
        )
    ]
...
def quality_dashboard(
    ...
    try:
        ...
        end = datetime.fromisoformat(date_to).replace(tzinfo=timezone.utc) if date_to else None
        if start and end and start > end:
            raise ValueError()
    except ValueError:
        raise HTTPException(422, "Use ISO dates in ascending order")
    return dashboard(db, user, project_id, start, end)

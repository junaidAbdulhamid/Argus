...
def calculate(db, task, store=True):
    ...
    fields = {}
    definitions = (
        db.scalars(select(AnnotationField).where(AnnotationField.schema_id == task.schema_id)).all()
        if task.schema_id
        else []
    )
    for field in definitions:
        ...
        if field.field_type in {"boolean", "single_select", "multi_select"}:
            fields[field.key] = {
                "kind": "categorical",
                **categorical([sorted(v) if isinstance(v, list) else v for v in values]),
            }
            ...
            fields[field.key] = {
                "kind": "unscored",
                "count": len(values),
                "note": "Text/structured values require human interpretation",
            }
    summary = {
        ...
        "scores": scores,
        "fields": fields,
        "conflicting": labels["conflicting"] or any(v.get("conflicting", False) for v in fields.values()),
        "cohen_kappa": None,
        "fleiss_kappa": None,
        "kappa_note": "Chance-corrected kappa needs a cohort of multiple tasks; see project analytics.",
        "interpretation": "Agreement is consistency, not evidence of correctness.",
    }
    if store:
        db.add(
            ConsensusResult(

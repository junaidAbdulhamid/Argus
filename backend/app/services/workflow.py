...
TRANSITIONS = {
    ...
    TaskStatus.PENDING_REVIEW: {TaskStatus.APPROVED, TaskStatus.REJECTED, TaskStatus.CHANGES_REQUESTED, TaskStatus.ESCALATED},
    ...
    TaskStatus.ESCALATED: {TaskStatus.PENDING_REVIEW, TaskStatus.ASSIGNED, TaskStatus.QUEUED, TaskStatus.REJECTED, TaskStatus.CHANGES_REQUESTED},
    TaskStatus.APPROVED: {TaskStatus.PENDING_REVIEW, TaskStatus.REJECTED, TaskStatus.CHANGES_REQUESTED, TaskStatus.ESCALATED},
}
...
def recover(db, user):
    ...
    tasks = db.scalars(select(Task).join(Project).join(AnnotationAssignment).where(
    ...
    ).with_for_update(of=Task, skip_locked=True)).unique().all()
    for task in tasks:
        expired = db.scalars(select(AnnotationAssignment).where(
        ...
        db.flush()
        active = db.scalar(select(AnnotationAssignment.id).where(AnnotationAssignment.task_id == task.id, AnnotationAssignment.status.in_([AssignmentStatus.ASSIGNED, AssignmentStatus.STARTED])).limit(1))
...
def claim(db, user, project_id=None):
    ...
    recover(db, user)
    existing = db.scalar(select(AnnotationAssignment).where(AnnotationAssignment.annotator_id == user.id, AnnotationAssignment.status.in_([AssignmentStatus.ASSIGNED, AssignmentStatus.STARTED])))
    if existing:
        return existing
    reservations = select(func.count(AnnotationAssignment.id)).where(
        AnnotationAssignment.task_id == Task.id,
        AnnotationAssignment.round == Task.annotation_round,
        AnnotationAssignment.status.in_([AssignmentStatus.ASSIGNED, AssignmentStatus.STARTED, AssignmentStatus.COMPLETED]),
    ).correlate(Task).scalar_subquery()
    previous = select(AnnotationAssignment.id).where(
        AnnotationAssignment.task_id == Task.id, AnnotationAssignment.annotator_id == user.id,
        AnnotationAssignment.round == Task.annotation_round,
        AnnotationAssignment.status != AssignmentStatus.EXPIRED,
    ).correlate(Task).exists()
    query = select(Task).join(Project).where(
        Project.organization_id == user.organization_id, Project.status == "ACTIVE",
        Task.status.in_([TaskStatus.QUEUED,TaskStatus.ASSIGNED]),
        reservations < Task.required_annotations, ~previous,
    )
    if project_id:
        query = query.where(Task.project_id == project_id)
    completed = db.scalar(select(func.count(AnnotationAssignment.id)).where(AnnotationAssignment.annotator_id == user.id, AnnotationAssignment.status == AssignmentStatus.COMPLETED)) or 0
    ...
    ordered=query.order_by(Task.priority.desc(),Task.created_at,Task.id).with_for_update(of=Task,skip_locked=True).limit(1)
...
def complete(db, user, assignment_id):
    ...
    db.flush()
    audit(db,user,"ANNOTATION_SUBMITTED",task,payload={"assignment_id":assignment.id,"round":task.annotation_round})
    grade(db,user,task,annotation)
    if len(completed_annotations(db,task))>=task.required_annotations:
        ...
        if consensus['conflicting'] and rules['auto_escalate_disagreement']:
            escalate(db,user,task,'Independent annotations disagree; human adjudication is required.','DISAGREEMENT')
    db.flush()
    return assignment

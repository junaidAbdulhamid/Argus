...
def schema_detail(db,schema):
    if schema is None:
        return None
    return {**serialize(schema),"fields":[serialize(f) for f in db.scalars(select(AnnotationField).where(AnnotationField.schema_id==schema.id).order_by(AnnotationField.position))]}


def active_schema(db,project_id):
    return db.scalar(select(AnnotationSchema).where(AnnotationSchema.project_id==project_id).order_by(AnnotationSchema.version.desc()).limit(1))


def create_schema(db,user,project,data):
    version=(db.scalar(select(func.max(AnnotationSchema.version)).where(AnnotationSchema.project_id==project.id)) or 0)+1
    ...
    audit(db,user,"ANNOTATION_SCHEMA_CREATED",project_id=project.id,payload={"schema_id":schema.id,"version":version})
    db.flush()
...
def validate_values(db,task,values):
    ...
    for field in fields:
        ...
        valid=(kind=="boolean" and isinstance(value,bool) or kind=="single_select" and isinstance(value,str) and value in field.options or kind=="multi_select" and isinstance(value,list) and all(isinstance(x,str) and x in field.options for x in value) and len(value)==len(set(value)) or kind=="integer_rating" and type(value) is int or kind=="continuous_score" and type(value) in (float,int) and math.isfinite(value) or kind=="text" and isinstance(value,str) or kind=="json" and isinstance(value,(dict,list)))
        if not valid:

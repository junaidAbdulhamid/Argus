...
def resolve(db, user, task, escalation, data):
    ...
    db.flush()
    if task.status == TaskStatus.ESCALATED and not db.scalar(
        select(Escalation.id).where(Escalation.task_id == task.id, Escalation.status == "OPEN")
    ):
        ...
        transition(
            ...
            if completed >= task.required_annotations
    ...
    audit(
        ...
        "ESCALATION_RESOLVED",
        task,
        payload={"escalation_id": escalation.id, "status": data.status, "resolution": data.resolution},
    )
    return escalation
...
def submit_review(db, user, task, decision, comments, metadata=None):
    ...
    if decision == "APPROVED" and db.scalar(
        select(Escalation.id).where(Escalation.task_id == task.id, Escalation.status == "OPEN")
    ):

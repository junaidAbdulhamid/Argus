...
def latest_reviews(db, task):
    reviews = db.scalars(
        select(Review)
        .where(Review.task_id == task.id, Review.round == task.annotation_round)
...
def evaluate(db, user, task):
    ...
    approved = [
        r
        for r in reviews
        if r.decision == "APPROVED" and r.reviewer_id not in authors and sorted(r.annotation_ids) == current_ids
    ]
    ...
    categorical_metrics = [
        consensus["labels"],
        *[v for v in consensus["fields"].values() if v["kind"] == "categorical"],
    ]
    if any(
        m["raw_agreement"] is not None and m["raw_agreement"] < rules["minimum_agreement"] for m in categorical_metrics
    ):
    ...
    if rules["maximum_variance"] is not None and any(
        m["variance"] is not None and m["variance"] > rules["maximum_variance"] for m in numeric_metrics
    ):
    ...
    if rules["minimum_gold_accuracy"] is not None:
        for author in authors:
            attempts = db.scalars(
                select(GoldAttempt)
                .where(GoldAttempt.annotator_id == author)
                .order_by(GoldAttempt.created_at.desc())
                .limit(20)
            ).all()
    ...
    row = QualityGateResult(
        ...
        policy=rules,
        evidence={
            "annotation_ids": current_ids,
            "review_ids": [r.id for r in approved],
            "consensus": consensus,
    ...
    audit(
        ...
        task,
        payload={"gate_result_id": row.id, "eligible": row.eligible, "reasons": row.reasons},
    )
    return row
